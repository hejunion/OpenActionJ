 ...
 */
package com.ypg.merchant.services.concurrent.workmanager.impl.nde;

import java.util.List;

import com.ypg.merchant.services.concurrent.workmanager.ext.ResultCollector;
 ...
 *
 */
public class NDEWorkItemImpl implements WorkItem, Runnable {
	 ...
	 * @param work the work
	 */
	public NDEWorkItemImpl(Work work, WorkListener wl) {
		this.work = work;
		this.wl = wl;
	}
	...
	public void run() {
		...
		if (work == null) {
			...
			if (wl != null) {
				wl.workRejected(new NDEWorkEventImpl(this, WorkEvent.WORK_REJECTED));
			}
			return;
		}
		status = WorkEvent.WORK_ACCEPTED;
		if (wl != null) {
			wl.workAccepted(new NDEWorkEventImpl(this, WorkEvent.WORK_ACCEPTED));
		}
		
		// start the work
		status = WorkEvent.WORK_STARTED;
		if (wl != null) {
			wl.workStarted(new NDEWorkEventImpl(this, WorkEvent.WORK_STARTED));
		}
		
		// run the work
		NDEWorkEventImpl event = new NDEWorkEventImpl(this, WorkEvent.WORK_COMPLETED);
		try {
			work.run();
		}
		catch (Throwable th) {
